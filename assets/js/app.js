...
/* ── Admin Centre sub-navigation ─────────────────────────────────
   One strip injected at the top of every admin page so the six
   admin areas feel like a single Admin Centre. */
var ADMIN_AREAS = [
  { p: 'site-admin',   t: 'Site',         icon: '<circle cx="12" cy="12" r="3"/><path d="M19.07 4.93l-1.41 1.41M12 2v2M4.93 4.93l1.41 1.41M2 12h2M4.93 19.07l1.41-1.41M12 22v-2M19.07 19.07l-1.41-1.41M22 12h-2"/>' },
  { p: 'ldc-admin',    t: 'L&amp;D',      icon: '<path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/>' },
  { p: 'rooms-admin',  t: 'Rooms',        icon: '<path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/>' },
  { p: 'ict-admin',    t: 'ICT',          icon: '<rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/>' },
  { p: 'forms-admin',  t: 'Forms',        icon: '<path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/>' },
  { p: 'form-builder', t: 'Form Builder', icon: '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/>' }
];

(function injectAdminSubnav() {
  ADMIN_AREAS.forEach(function(area) {
    var pg = document.getElementById('page-' + area.p);
    if (!pg || pg.querySelector('.admin-subnav')) return;
    var bar = document.createElement('div');
    bar.className = 'admin-subnav';
    bar.setAttribute('role', 'navigation');
    bar.setAttribute('aria-label', 'Admin Centre sections');
    bar.innerHTML = '<span class="admin-subnav-label"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>Admin Centre</span>'
      + ADMIN_AREAS.map(function(b) {
          return '<button class="admin-subnav-btn' + (b.p === area.p ? ' active' : '') + '" onclick="showPage(\'' + b.p + '\')">'
            + '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">' + b.icon + '</svg>' + b.t
            + '</button>';
        }).join('');
